...
dataList <- list(
    'y' = rbind(as.matrix(Yield.data),NA*rep(1,N.batches)),
    "N.batches"=N.batches,
    "N.samples"=N.samples+1)
# In the above I appended a row of NAs to the data in order to
# predict the yield in each batch
...
parameters <- c(
	...
	"ypredb5",
	"ypredb6") 
# Note that the data I wish to predict are listed above
...
jagsModel <- jags.model("model-with-postpred.txt", 
				data=dataList, 
				inits=initsValues, 
				n.chains=nChains, 
				n.adapt=adaptSteps)
# Note that the model name is different
