...
#Number of regions
N.batches <- length(levels(Region))    
#Number of samples of each region
N.samples <- numeric(N.batches)
i=1
for (r in levels(data$Region)){
  N.samples[i]<-length(which(data$Region==r))
  i<-(i+1)
}

# Create a list contining the data (note: use meaningful variable names)
dataList <- list(
    'y' = as.matrix(salmon.data$S),
    "N.batches"=N.batches,
    "N.samples"=N.samples)
